...
class CheetahInterface:
    XML         = Path(INTERFACE_PATH / 'cheetah.xml')
    DEFAULT_FF  = [0.0, 0.0, 0.0]
    DEFAULT_JT  = [0.0] * 6

class Cheetah(mm.envs.SwappableBase):
    """Hopper environment."""
    ...
    def reset(self, rng: jax.Array) -> mm.EnvState:
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mm.EnvState:
